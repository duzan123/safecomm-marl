...
|------|------|------|----------|------------|
| Phase 1 | Scheduler + Prototype | ✅ 已完成 | — | `docs/superpowers/plans/2026-05-26-safecomm-voi-phase1.md` |
...
**WandB 验证**：
- [ ] `train.py` 包含 `wandb.init(project="safecomm-marl", ...)`，运行 3 步 dry-run 后 WandB 可见 run 记录
- [ ] smoke test 文件顶部设置 `WANDB_MODE=disabled`，`pytest tests/ -v` 不受 WandB 影响
...
- `train.py`：WandB 集成正确，API key 通过环境变量注入，无硬编码
...
**约束 13：WandB 训练追踪（Phase 2 起强制）**  
所有正式训练（非 smoke test）必须初始化 WandB 并记录 metrics。  
- `wandb.init(project="safecomm-marl", config=cfg, name=f"{AlgoName}-N{n}-k{k}-{timestamp}")`  
- 每次迭代调用 `wandb.log({...metrics...}, step=agent.total_steps)` 并最终调用 `wandb.finish()`  
- 必须记录：`actor_loss`、`lambda_s`、`J_safe`、`approx_kl`、`mean_episode_cost`、`mean_formation_error`、`mean_bandwidth_util`（Phase 2 起额外：`lambda_int`、`safe_gate_active`、`J_int`）  
- Smoke test 和单元测试中通过 `WANDB_MODE=disabled` 禁用 WandB，不得影响测试通过  
- API Key 存储在 `.env` 文件（已加入 `.gitignore`），通过 `WANDB_API_KEY` 环境变量注入，**禁止硬编码在任何源文件中**
...
| 阶段 | 状态 | 完成日期 | 备注 |
|------|------|----------|------|
| Phase 1 | ✅ 已完成 | 2026-05-27 | 80 tests passing，门控通过 |
| Phase 2 | ⬜ 待完成 | — | 依赖 Phase 1（✅ 已满足） |
